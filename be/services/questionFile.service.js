...
async function addQuestion(questionFileId, questionData) {
  const newQuestion = await questionRepository.addQuestion(questionFileId, questionData);
  return {
    questionId: newQuestion._id, // Trả về _id của câu hỏi mới
...
async function deleteQuestion(questionFileId, questionId) {
  // Gọi repository để xóa câu hỏi
  const updatedQuestionFile = await questionRepository.deleteQuestion(
    questionFileId,
    questionId
  );
  return updatedQuestionFile;
}
