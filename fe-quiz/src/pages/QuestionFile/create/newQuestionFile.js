...
const QuestionCreator = () => {
    ...
    setQuestions(
      questions.map((question) => {
        if (question.id === questionId) {
          ...
          return { ...question, type: newType, selectedAnswers: newSelectedAnswers };
        }
        return question;
      })
    );
      ...
      questions.map((question) => {
        if (question.id === questionId) {
          ...
          if (question.type === "trueFalse" || question.type === "singleAnswer") {
            ...
            else newSelectedAnswers = newSelectedAnswers.filter((i) => i !== answerIndex);
          }
  ...
  const addQuestion = () => {
    ...
    setQuestions([
      ...questions,
      { id: newId, type: "trueFalse", answers: ["Đúng", "Sai"], selectedAnswers: [], question: "" },
    ]);
  };
  ...
  const addAnswer = (questionId) => {
    setQuestions(
      questions.map((q) => {
        if (q.id === questionId) {
          return { ...q, answers: [...q.answers, `Đáp án ${q.answers.length + 1}`] };
        }
        return q;
      })
    );
  ...
  const handleSubmit = async () => {
    ...
    const payload = { name: title, description, arrayQuestion: formattedQuestions };
  ...
  const parseTxtFile = (fileContent) => {
    ...
    lines.forEach((line, index) => {
      ...
      } else if (line && (line.match(/^\d+\./) || (!line.match(/^[a-e]\./) && line.length > 0))) {
        // Phát hiện câu hỏi mới (ưu tiên số thứ tự hoặc không phải đáp án với nội dung)
        if (currentQuestion) {
  ...
  const handleFileImport = (event) => {
    ...
    reader.onload = (e) => {
      ...
      try {
        ...
        if (window.confirm("Bạn có chắc muốn import file này? Hành động này sẽ tải dữ liệu lên giao diện.")) {
          setTitle(parsedData.name || title);
          setDescription(parsedData.description || description);
          setQuestions(
            parsedData.arrayQuestion.map((q, index) => ({
              id: index + 1,
              type: q.type === "Boolean" ? "trueFalse" : q.type === "MAQ" ? "multiAnswer" : "singleAnswer",
              question: q.content,
              answers: q.answers.map((a) => a.answerContent),
              selectedAnswers: q.answers.reduce((acc, a, idx) => (a.isCorrect ? [...acc, idx] : acc), []),
            }))
          );
          setIsDirty(true);
  ...
  const renderQuestionRow = ({ index, key, style }) => {
      ...
      <div key={key} style={{ ...style, position: "relative" }}>
        <QuestionCard>
          <QuestionHeader>
            ...
            <Select value={question.type} onChange={(e) => handleTypeChange(question.id, e.target.value)}>
          ...
          <AnswersGrid>
            {question.answers.map((answer, aIndex) => (
              <AnswerItem key={aIndex}>
                <RadioInput
                  type={question.type === "multiAnswer" ? "checkbox" : "radio"}
                  name={`question-${question.id}`}
                  checked={question.selectedAnswers.includes(aIndex)}
                  onChange={(e) => handleAnswerChange(question.id, aIndex, e.target.checked)}
                />
                <InputField
                  type="text"
                  value={answer}
                  onChange={(e) => updateAnswerText(question.id, aIndex, e.target.value)}
                  placeholder={`Đáp án ${aIndex + 1}`}
                />
                {question.answers.length > 2 && (
                  <DeleteButton onClick={() => removeAnswer(question.id, aIndex)}>
                    <Trash2 size={16} />
                  </DeleteButton>
                )}
              </AnswerItem>
  ...
  return (
    <Container>
      ...
      />
      <ImportButtonContainer>
        <label htmlFor="file-import" style={{ cursor: "pointer", display: "inline-block" }}>
      ...
      >
        <ModalContent>
          ...
          <ul>
            <li><strong>Chủ đề:</strong> [Tên chủ đề]</li>
            <li><strong>Mô tả:</strong> [Mô tả]</li>
            <li><strong>Công khai:</strong> Có/Không</li>
            <li><strong>Câu hỏi:</strong> [Số]. [Nội dung câu hỏi] ([Boolean/MAQ/MCQ]) hoặc chỉ [Nội dung câu hỏi]</li>
            <li><strong>Đáp án:</strong> [a-e]. [Nội dung đáp án] ([Đúng/Sai]) hoặc chỉ [Nội dung đáp án]</li>
          </ul>
          <p><strong>Ví dụ định dạng chuẩn:</strong></p>
          ...
          <p><strong>Ví dụ định dạng linh hoạt:</strong></p>
            ...
            Trung Quốc <br></br>
            ..... 
          <p><strong>Lưu file với định dạng .txt và import để sử dụng!</strong></p>
        </ModalContent>
      </Modal>
